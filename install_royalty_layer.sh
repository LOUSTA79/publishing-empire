...
success() { printf "\033[1;32m✅ %b\033[0m\n" "$*"; }

say "╔════════════════════════════════════════════════════════════╗"
say "║   💰 LOUGROK SOVEREIGN — ROYALTY PAYOUT AUTOMATION v1.0    ║"
say "║   70% Author Royalty • 100% Autonomous • Audit-First       ║"
say "╚════════════════════════════════════════════════════════════╝"

mkdir -p data

# ---- Initial royalty ledger (source of truth) ----
if [ ! -f data/royalty_ledger.json ]; then
cat > data/royalty_ledger.json << 'LEDGER'
{
  "version": "1.0",
  "authors": {
    "default": {
...
LEDGER
success "Created data/royalty_ledger.json"
fi

# ---- Update .env with new royalty controls ----
if ! grep -q ROYALTY_PAYOUT_THRESHOLD .env 2>/dev/null; then
  cat >> .env << 'ENVADD'

# === Royalty Payout Automation (LouGrok Sovereign) ===
ROYALTY_PAYOUT_THRESHOLD=25
ROYALTY_AUTONOMOUS_ENABLED=false
# Set to true only after full testing in TEST mode
# STRIPE_WEBHOOK_SECRET=whsec_xxx   (add later from Stripe dashboard)
ENVADD
  success "Added royalty controls to .env"
fi

# ---- Enhanced server.js with full royalty layer + autonomous engine ----
...
success "server.js upgraded with full royalty automation + autonomous engine"

# ---- royalty_payout_engine.js (standalone version for Termux/PM2 flexibility) ----
cat > royalty_payout_engine.js << 'ENGINE'
#!/usr/bin/env node
...
ENGINE
chmod +x royalty_payout_engine.js
success "royalty_payout_engine.js created (standalone + --dry-run support)"

# ---- PM2 runner using your exact bluebot pattern ----
cat > royalty_pm2_runner.sh << 'RUNNER'
#!/data/data/com.termux/files/usr/bin/bash
while true; do
  date
...
RUNNER
chmod +x royalty_pm2_runner.sh
success "royalty_pm2_runner.sh created (exact pattern from your bluebot_pm2_runner.sh)"

# ---- Quick CLI ----
cat > royalty_cli.sh << 'CLI'
#!/data/data/com.termux/files/usr/bin/bash
cd "$(dirname "$0")"
case "$1" in
...
CLI
chmod +x royalty_cli.sh
success "royalty_cli.sh created"

success "All files installed. Now commit & push to Railway, then set the two new env vars in Railway dashboard:"
echo "   ROYALTY_PAYOUT_THRESHOLD=25"
echo "   ROYALTY_AUTONOMOUS_ENABLED=false   (flip to true only after testing)"
echo ""
echo "Test flow:"
echo "1. Set PRODUCTION_MODE=false, redeploy"
echo "2. Create a test payment via /api/payment with author_id"
echo "3. On Termux: ./royalty_cli.sh dry-run"
echo "4. When ready → set flags true + redeploy → engine runs forever autonomously"
